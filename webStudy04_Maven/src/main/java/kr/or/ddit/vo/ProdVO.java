 ...
 * 
 * ** iBatis 나 MyBatis 를 이용해 여러 테이블로부터 데이터를 조회하는 방법.
 * 1. 메인데이터를 중심으로 테이블간의 관계 파악
 *   ex) 상품정보 상세조회(조회할 상품의 구매자 목록을 함께 조회)
 *   	상품(1) : 구매자(N)
 * 2. 각 테이블로부터 데이터 조회시 사용할 VO 를 구현.
 *    각 VO 간의 관계성을 설정.
 ...
 * 		outerVO(ProdVO) 와 innerVO(BuyerVO, MemberVO) 역할의 resultMap 들이 중첩되도록 설정.
 *   주의! 1:N 관계 조인시, 여러건의 레코드에서 중복을 제거해야함(groupBy).
 * 3-2. 각테이블로부터 데이터를 조회할 쿼리를 분리 작성.
 *      ex) selectProd, selectCusomers
 * 4-2. Nested Select 방식 : resultMap 구현시, 
 * 		 outerVO  가 가진 특정 프로퍼티를 바인딩할 설정에서 select 속성을 사용하여, 추가 쿼리문을 실행함.
 *      ex) <result property="customers" select="Prod.selectCustomers" column="PROD_ID" />
 *   주의! 중첩된 select 문에 인라인 파라미터가 사용된다면, result 설정의 column 속성을 통해 값을 전달.   
