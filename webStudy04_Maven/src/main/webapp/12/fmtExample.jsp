...
<body>
   ...
   <c:set var = "clientTimezone" value = "${dateFn:getDefault() }"/>
   <c:choose>
      <c:when test="${not empty param.locale }">
         <c:set var="pageLocale"
            value="${clientLocale.forLanguageTag(param.locale) }" />
      </c:when>
   ...
   <table>
      ...
      <tbody>
         <tr>
            ...
            <td><fmt:formatDate value="${dateFn:getNow() }" type="both" /></td>
            <td>
               ${pageTimeZone.displayName }
            <br>
            <fmt:formatDate value="${dateFn:getNow() }" type="both" timeZone="${pageTimeZone }"/></td>
         </tr>
      </tbody>
   </table>
</body>
