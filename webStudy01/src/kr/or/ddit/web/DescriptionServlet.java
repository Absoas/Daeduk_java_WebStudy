...
/**
 * Servlet 
 * : 자바 기반의 확장 CGI 방식을 이용한 웹 프로그램이 기술 스펙이면서 동시에 API.
 * 
 *  서블릿 개발 단계
 *  1. HttpServlet을 상속하여 구현.
 *  2. 컴파일 -> 웹 어플리케이션의 classpath 에 배포(/WEB-INF/classes)
 *  3. 등록 
 ...
 *  	2) @WebServlet을 이용한 등록(since 3.0)
 *  
 *  5. 서버(WAS, ServletContainer) 재구동
 *  
 *  W.A.S (Web Application Server, ServletContainer, WebContainer, JSPContainer)
 *   : 서블릿 스펙에 따라 개발된 웹 어플리케이션을 운영할 주체(관리자).
 *   : 서블릿의 생명주기를 관리하는 역할 
 *   (서블릿의 생명주기 내에서 특정 이벤트가 발생시 해당 콜백 메소드를 호출)
 *   : 싱글턴 , 요청이 쓰레드로 처리되며, 해당 쓰레드는 풀링 정책에 따라 관리됨.
 *   
 *   서블릿의 생명주기와 관련된 콜백 메소드의 종류
 *   	init : 서블릿의 객체가 생성된 직후에 호출되는 메소드
 *   	service : 요청이 발생할때마다 반복 호출
 *   			  요청의 HTTP method 에 따라 doXXX 계열 메소드를 호출.
 *   	doGet 등의 doXXX 계열 메서드
 *   			: 요청이 발생할때마다 반복 호출
 *   				Http method에 따라 service에 의해 호출됨.
 *   	destroy : 서블릿의 객체가 소멸되기 직전에 호출되는 메소드
 *   	
 *   WAS 의 서블릿을 이용한 요청 처리 단계
 *   1) 서블릿 매핑 정보를 통해 현재 요쳥을 처리할 서블릿을 검색.
