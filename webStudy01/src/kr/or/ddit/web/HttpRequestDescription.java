 ...
 * 	GET		: 조회		 		(R)
 * 		Body 가 존재하지 않기 때문에, 전송할 최소한의 데이터를 Line 의 URL에 덧붙임.
 * 		ex)http://localhost/webStudy01/gugudan.do?minDan=4&maxDan=5
 * 		주소 : 포트 넘버/세부경로?QueryString 
 * 		QueryString : segment1&segment2....
 * 		segment : parameterName = parameterValue		
 * 		
 * 	POST	: (신규)전송 및 등록	(C)
 * 	PUT		: 전송 및 수정 		(U)
 *  DELETE 	: 삭제 				(D)
 ...
 *  HEAD	: 응답 데이터를 body 가 없는 형태로 받는다.
 *  
 * Request Header : 클라이언트와 요청에 관한 부가 정보(metadata)
 * 		헤더명 : 헤더값 .... 
 * Request Body(Contents, Message body) : method 가 POST 일 경우. 형성.
 ...
 *   	IETF에서 정의한 RFC2396 규약에 따라 모든 특수문자는 URLEncoding(Percent Encoding) 방식으로 인코딩되어 전송.
 *		인코딩된 특수문자 확보 
 *		GET		:	서버의 설정을 통한 디코딩 설정. 
 *			server.xml -> URIEncoding :	특정 인코딩 방식으로 고정.
 *						  useBodyEncodingForURI : POST방식에서 사용하는 request.setCharacterEncoding(encoding)을 지원함.
 *
 *		POST	:	request.setCharacterEncoding(encoding)  - 파라미터를 꺼내기전에 설정함.
 * 
 * 2) 요청 헤더의 확보
