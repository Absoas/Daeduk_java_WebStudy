...
<h4>쿠기의 속성들</h4>
<pre>
	1. path				: 브라우저의 쿠키 저장소에 저장된 쿠키가 다시 서버로 재전송 될지 여부
						    쿠키에 설정된 path 속성의 값을확인하여, 
						    해당 경로의 하위 경로로 발생하는 요청에만 쿠키가 재전송.
						    생략한 경우 , 쿠키의 생성 경로를 기본값으로 사용함.
						 
	2. domain / host	: 저장된 쿠키의 재전송 여부를 결정할 때 사용. 생략한 경우 , 쿠키가 생성된 도메인(호스트)가 사용됨.
						    특정 사이트가 보유한 모든 호스트에 전송될 쿠키를 생성할 떄 사용.
						    도메인의 레벨 구조에 따라 ".naver.com" 방식으로 설정함.
						
	3. maxAge			: 만료 시간 , 생략하면 , 기본값으로 세션의 만료시간이 반영됨.
					-1 : 브라우저의 종료 시점에 삭제.
					0  : 기존의 쿠기가 있을 경우, 해당 쿠키를 삭제할때 사용.
	...
	<%
		String allPathCookieValue =  URLEncoder.encode("모든 경로에서 사용할 수 있음","UTF-8");
		...
		Cookie otherPathCookie = new Cookie("otherPathValue",otherPathCookieValue);
		otherPathCookie.setPath(request.getContextPath()+"/09");
		response.addCookie(otherPathCookie);
		
// 		Cookie otherDomainCookie = new Cookie("otherDomainCookie","otherDomainCookie");
// 		otherDomainCookie.setDomain(".kyb.com");
