 ...
 * 			 - Exception (checked exception): 경우에 따라 직접 처리할 수도 잇는 상황.
 *				-RuntimeException (unckecked exception) : 직접 처리할수도 있지만, 명시적으로 처리하지 않는 경우 , JVM이 제어권을 획득함
 *
 *	1) checked exception : IOException, SQLException, ServletException
 *		발생가능성이 있는 코드에서 예외를 처리하지 않을 경우, 컴파일 에러.
 *	2) unchecked exception : Null - Illegal - ArithmeticException
 *
 *	** 예외 처리 방법
 *	1) try ~ catch ~ finally	: 예외 발생시 직접 처리 
 *	2) throws					: 예외 발생 시 호출자에게 위임
 *	
 */
